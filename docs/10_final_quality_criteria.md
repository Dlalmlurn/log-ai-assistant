...
- [ ] baseline 支持全局、滚动、星期和月度周期中的至少三种。
- [ ] 检测时能记录实际选中的周期 baseline、fallback 和 override。
- [ ] 样本不足时有 peer group 或全局 baseline 兜底。
- [ ] 异常事件能展示 baseline 偏离原因。
- [ ] 授权用户可以手动追加、设置有效期并撤销 baseline override。
...
- [ ] AI 反馈写入 `ai_feedback`。
- [ ] AI 反馈未经审核不修改生产规则或 effective baseline。
- [ ] 接受面向 baseline 的反馈后生成版本化 override，并保留审核人与关联反馈。
- [ ] 拒绝、撤销或过期的 override 不再影响检测结果。
- [ ] mock 输出必须明确标记。
...
- [ ] 周期任务由 Compose 内统一运营控制面调度。
- [ ] baseline、数据质量对账和日报能按业务日期自动运行。
- [ ] 每次任务运行记录幂等键、数据水位、输入版本、状态、输出和失败原因。
- [ ] 重复执行同一幂等键不会生成重复业务结果。
- [ ] 失败任务可以重试，且保留每次 attempt 的审计记录。
- [ ] 数据水位不足时不生成正式 baseline 或日报。

## 量化验收标准

- [ ] 验收报告分别统计正常误报率、攻击检出率和高危检出率。
- [ ] 验收报告统计 high/critical precision、攻击事件 recall、攻击类型混淆矩阵和风险等级准确率。
- [ ] 验收报告统计端到端可追踪率、检测延迟和通知延迟。
- [ ] 报告保存代码、场景、规则、baseline、AI 模型和阈值版本。
- [ ] mock AI 与真实模型结果分开统计。
- [ ] 无法解释的数据阶段差异进入 `needs_review`，不标记为通过。
- [ ] high/critical 异常通过持久化 outbox 生成通知任务。
- [ ] 通知失败可重试，且不影响 `AnomalyEvent` 落库和查询。
